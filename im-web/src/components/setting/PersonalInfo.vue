	...
	<div class="personal-info">
				...
				<div class="avatar-section">
					<div class="avatar-container">
						...
						<div class="upload-overlay">
							...
							<span>{{ $t('personalInfo.changeAvatar') }}</span>
						</div>
					</div>
				</div>
				<div class="user-basic-info">
					<h3 class="user-name">
						{{ userInfo.nickName || userInfo.userName || $t('personalInfo.noNickname') }}
						<span class="company-tag" v-if="userInfo.companyName">@{{ userInfo.companyName }}</span>
					</h3>
					<p class="user-id">{{ $t('common.userName') }}: {{ userInfo.userName || $t('home.unknownUser') }}
					</p>
					<p class="user-id">ID: {{ userInfo.code }}</p>
				</div>
			</div>
		</div>
		...
		<div class="form-section">
			<div class="title-box">
				<h4 class="section-title">
					...
					{{ $t('personalInfo.inviteCode') }}
				</h4>
				<el-button circle @click="openInviteCode = !openInviteCode">
					<i class="el-icon-arrow-up" v-if="openInviteCode"></i>
					<i class="el-icon-arrow-down" v-else></i>
				</el-button>
			</div>

			<div class="invite-code" v-if="openInviteCode">
				<vue-qr class="qrcode" :text="qrText" :size="280" :margin="20" :callback="getQrBase64" />
				<div class="h-box">
					<el-button type="primary" v-if="qrBase64" @click="downloadQr">{{ $t('personalInfo.download') }}</el-button>
				</div>
			</div>
		</div>
		...
		<div class="form-section">
			...
			<el-form :model="userInfo" label-width="80px" :rules="rules" ref="personalForm" size="small">
				<el-form-item :label="$t('personalInfo.company')" v-if="userInfo.companyName">
					<span class="company-text">{{ userInfo.companyName }}</span>
				</el-form-item>
				<el-form-item prop="nickName" :label="$t('personalInfo.nickname')">
					<el-input v-model="userInfo.nickName" autocomplete="off" size="small" maxlength="32" show-word-limit
						:placeholder="$t('personalInfo.inputNickname')"></el-input>
				</el-form-item>
				<el-form-item :label="$t('personalInfo.sex')">
                ...
                <div class="contact-item">
                    ...
                    <div class="contact-content">
                        ...
                        <div v-else class="bind-section">
                            <span class="no-bind-text">{{ $t('personalInfo.noBindPhone') }}</span>
                            <div type="text" @click="onBindPhone" class="bind-btn">
                                <i class="el-icon-link"></i>
                                {{ $t('personalInfo.goBind') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="contact-item">
                    ...
                    <div class="contact-content">
                        <el-input v-if="userInfo.email" disabled v-model="userInfo.email" autocomplete="off"
                            size="small"></el-input>
                        <div v-else class="bind-section">
                            <span class="no-bind-text">{{ $t('personalInfo.noBindEmail') }}</span>
			...
			<el-form :model="userInfo" ref="personalForm" size="small">
				<el-form-item>
					<el-input type="textarea" v-model="userInfo.signature" :rows="3" maxlength="64" show-word-limit
						:placeholder="$t('personalInfo.signaturePlaceholder')"></el-input>
				</el-form-item>
			</el-form>
		</div>
		...
		<div class="btn-group">
			<el-button type="primary" @click="onSubmit()">{{ $t('personalInfo.submit') }}</el-button>
		</div>
	</div>
</template>
		...
		methods: {
			init() {
				...
				if (!mine || !mine.id) {
					...
					this.qrText = `&scan=1&userId=${this.userInfo.id}&serverCode=${this.userInfo.serviceCode}`
				}
			},
			onSubmit() {
				this.$refs.personalForm.validate((valid) => {
					...
					this.$http({
						...
						this.$message.success(this.$t('personalInfo.modifySuccess'));
					})
				});
			},
