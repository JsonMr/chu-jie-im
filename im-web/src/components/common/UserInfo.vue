	...
	<div v-if="show" class="user-info" :style="{ left: pos.x + 'px', top: pos.y + 'px' }" @click.stop>
		<div class="user-card">
			...
			<div class="user-header">
				...
				<div class="user-basic-info">
					<div class="user-name-row">
						...
						<el-tag v-if="user.status == 1" type="danger" size="mini">{{ $t('friend.cancelled') }}</el-tag>
						<el-tag v-if="user.isBanned" type="danger" size="mini">{{ $t('group.banned') }}</el-tag>
					</div>
					<div class="user-id">
						<span>{{ $t('common.userName') }}: {{ user.userName }}</span>
						<i class="el-icon-copy-document copy-btn" @click="copyUserName(user.userName)"
							:title="$t('chat.copyUserId')"></i>
					</div>
					<div class="user-id">
						<span>ID: {{ user.code }}</span>
						<i class="el-icon-copy-document copy-btn" @click="copyUserName(user.code)"
							:title="$t('chat.copyUserId')"></i>
					</div>
				</div>
			</div>
			...
			<div class="signature-section">
				<div class="signature-label">{{ $t('friend.signature') }}</div>
				<div class="signature-content">
					{{ user.signature ? user.signature : $t('friend.noSignature') }}
				</div>
			</div>
			...
			<div class="action-section">
				...
				<div v-else-if="isWaitingApprove" class="wait-status">
					...
					{{ $t('friend.waitingApprove') }}
				</div>
				<el-button v-else-if="user.id != mine.id && showAddFriend" type="success" size="small"
					@click="onAddFriend()" class="action-btn" icon="el-icon-plus">
					{{ $t('friend.addAsFriend') }}
				</el-button>
			</div>
		</div>
...
export default {
	...
	computed: {
		...
		group() {
			let groupId = this.chatStore.activeChat.targetId;
			return this.groupStore.findGroup(groupId);
		},
		isOwner() {
		...
		isManager() {
			...
			let m = this.group.members.find((m) => m.userId == userId);
			return m && m.isManager;
		},
		showAddFriend() {//判断显示加为好友按钮
			if (this.chatStore.activeChat.type == 'GROUP') {
				if (this.isOwner || this.isManager || this.group.isAllowAddFriend) {
					return true;
				} else {
					return false;
				}
