	...
	export default {
		...
		methods: {
			...
			async handlePaste() {
				...
				// 判断环境
				let platform = (navigator.userAgent.match(/(Electron)/i) || (/macintosh|mac os x/i.test(navigator
					.userAgent))) && !!(window.ipcRenderer && window.shell && window.remote)
				//根据环境选择相关
				let clipboard = platform.platform ? window.clipboard : window.navigator.clipboard
				console.log('platform、clipboard', platform, clipboard)
				...
				// 		// 	clipboard
				// 		// } = require('electron');
				// 		// Vue 2 在 Electron 渲染进程中，建议使用 window.requireNode
				// 		// 兼容性更好的引入写法
				...
				// 		console.log('环境检测2:', this.isElectron, '是否获取到文本:', text)
				// 		// Electron 可以直接读取图片
