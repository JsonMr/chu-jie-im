	...
	<el-container class="friend-page">
		<resizable-aside :default-width="260" :min-width="200" :max-width="500" storage-key="friend-aside-width">
			<div class="header" :class="configStore.electronMode ? 'header-menu-wrap' : ''">
				...
				<el-button plain class="add-btn" icon="el-icon-plus" :title="$t('friend.addFriend')"
			...
			<el-scrollbar class="friend-items">
				...
				<div class="top-item" :class="showFriendReuqest ? 'active' : ''" @click="onClickNewFriend">
					...
					<div class="top-item-info">
						<div class="top-item-name">{{ $t('friend.newFriend') }}</div>
					</div>
				</div>
		...
		<el-container v-if="showFriendReuqest" class="container">
			<div class="header" :class="configStore.electronMode ? 'header-menu-wrap' : ''">{{ $t('friend.newFriend') }}
			</div>
				...
				<div class="info-card">
					...
					<div class="info-section">
						<div class="name-header">
							...
							<div v-if="isFriend" :title="$t('common.more')" class="more-btn el-icon-more"
								@click="onClickMore"></div>
						</div>
						<div class="info-item-list">
							<div class="info-item">
								...
								<span class="info-value">
									{{ userInfo.userName }}
									<el-tag v-if="userInfo.status == 1" type="danger" size="mini">{{
										$t('friend.cancelled') }}</el-tag>
								</span>
							</div>
							<div class="info-item">
							...
							<div class="info-item">
								<span class="info-label">{{ $t('friend.nickname') }}</span>
								<span class="info-value">{{ userInfo.nickName }}</span>
							</div>
							<div class="info-item" v-if="userInfo.companyName">
								<span class="info-label">{{ $t('friend.company') }}</span>
								<span class="info-value">
									{{ userInfo.companyName }}
								</span>
							</div>
							<div v-if="isFriend" class="info-item remark-item">
								<span class="info-label">{{ $t('friend.remark') }}</span>
								<span class="info-value remark-content"
									:class="activeFriend.remarkNickName ? '' : 'light-text'">
									{{ activeFriend.showNickName || $t('friend.noRemark') }}
									<i class="icon iconfont icon-modify" @click="onEditRemark"></i>
								</span>
							</div>
							<div class="info-item">
								<span class="info-label">{{ $t('friend.sex') }}</span>
								<span class="info-value">{{ userInfo.sex == 0 ? $t('friend.male') : $t('friend.female')
								}}</span>
							</div>
							<div class="info-item">
								<span class="info-label">{{ $t('friend.signature') }}</span>
								<span class="info-value" v-if="userInfo.signature">{{ userInfo.signature }}</span>
								<span class="info-value light-text" v-else>{{ $t('friend.noSignature') }}</span>
							</div>
						</div>
						<div class="btn-group">
							<el-button v-show="isFriend" icon="el-icon-position" type="primary"
								@click="onSendMessage(activeFriend)">{{ $t('friend.sendMessage') }}</el-button>
							<el-button v-show="!isFriend" icon="el-icon-plus" type="primary"
								@click="onAddFriend(userInfo)">{{ $t('friend.addAsFriend') }}</el-button>
							<el-button v-show="isFriend" icon="el-icon-delete" type="danger"
								@click="onDelFriend(userInfo)">{{ $t('friend.deleteFriend') }}</el-button>
						</div>
					</div>
				</div>
			</div>
		...
		<chat-selector ref="chatSel" :title="$t('friend.sendCard')"></chat-selector>
	...
	export default {
		...
		data() {
			return {
				...
				menuItems: [{
					key: 'CHAT',
					name: this.$t('friend.sendMessage'),
				}, {
					key: 'CARD',
					name: this.$t('friend.sendCard'),
				}, {
					key: 'REMARK',
					name: this.$t('friend.modifyRemark'),
				}, {
					key: 'COMPLAINT',
					name: this.$t('complaint.report')
				}, {
					key: 'DELETE',
					name: this.$t('friend.deleteFriend'),
					danger: true
				}]
			}
		},
		...
		methods: {
			...
			onDelFriend(friend) {
				this.$confirm(this.$t('friend.deleteFriendConfirm', {
					name: friend.nickName
				}), this.$t('friend.confirmRemove'), {
					...
					this.$http({
						...
						console.log("this.$t('friend.deleteFriendSuccess'):", this.$t(
							'friend.deleteFriendSuccess'))
						this.$message.success(this.$t('friend.deleteFriendSuccess'));
				...
				this.$refs.chatSel.open(chats => {
					...
					chats.forEach(chat => {
						...
						this.$http({
							...
							if (++idx == chats.length) {
								this.$message.success(this.$t('friend.sendSuccess'))
							}
						})
					})
				})
			...
			onEditRemark() {
				this.$prompt(this.$t('friend.inputFriendRemark'), this.$t('friend.modifyRemark'), {
					inputValue: this.activeFriend.showNickName,
					confirmButtonText: this.$t('common.ok'),
					cancelButtonText: this.$t('common.cancel'),
					inputValidator: (value) => {
						if (value && value.length > 32) {
							return this.$t('friend.remarkMax32');
						}
					}
				}).then((o) => {
					let data = {
		...
		computed: {
			...
			friendMap() {
				...
				const onlineKey = this.$t('friend.online');
